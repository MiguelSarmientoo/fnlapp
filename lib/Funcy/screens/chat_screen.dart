...
Future<void> _fetchMessages({bool isLoadMore = false}) async {
    ...
    final url = Uri.parse(
        'http://18.225.10.73:3000/api/messages/filtered?userId=${widget.userId}&limit=$_limit&offset=$_offset');
    try {
  ...
  Future<void> _sendMessage(String text) async {
    final url = Uri.parse('http://18.225.10.73:3000/api/guardarMensaje');
    try {
  ...
  final url = Uri.parse('http://18.225.10.73:3000/api/ask');
  try {
    final response = await http.post(
      url,
      headers: {
